...
Docker Desktop 自体の再起動やスリープ復帰後、Grafana だけ復帰して otel-collector / prometheus / loki が `Exited(127)` のまま取り残されることがあります。復旧は `docker compose up -d` 単体だと上記の「マウント失効」エラーが出ることがあるため、必ず:
